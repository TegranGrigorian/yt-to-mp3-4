...
impl OSUtil {
    ...
    //determine default output folder based on the OS
    pub fn get_output_folder() -> PathBuf {
        if cfg!(target_os = "windows") {
            // Use the Music folder on Windows, preferec to keep user friendlness i guess
